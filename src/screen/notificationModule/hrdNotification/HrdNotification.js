...
const HrdNotification = ({ navigation }) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      {data[0]?.notification_type !== 6 ? <CenteredText text={AppString.NO_NOTIFICATION_FOUND} /> : null}
      <FlatList
        data={data}
        showsVerticalScrollIndicator={false}
        keyExtractor={(item, index) => item + index}
        renderItem={({ item }) => {
          return item?.notification_type == 6 ?
            <Swipeable
              key={item?.id}
...
export default HrdNotification;
